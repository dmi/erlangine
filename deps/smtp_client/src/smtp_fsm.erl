...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Usage Example (debug on)
%%
%% 1> c(smtp_fsm).
%% {ok,smtp_fsm}
%% 2> {ok,Pid} = smtp_fsm:start("172.24.75.203").
%% smtp_fsm,305 : connecting to "172.24.75.203"
%% smtp_fsm,309 : socket open
%% {ok,<0.45.0>}
%% 3> smtp_fsm:ehlo(Pid).
%% {ok,"250-smtp.test.co.uk Hello test.test.co.uk [172.24.72.71]
%%   \r\n250-SIZE\r\n250-PIPELINING\r\n250-AUTH PLAIN LOGIN 
%%   CRAM-MD5\r\n250-HELP\r\n"}
%% 4> smtp_fsm:features(Pid). 
%% {ok,["SIZE","PIPELINING","AUTH PLAIN LOGIN CRAM-MD5","HELP"]}
%% 5> smtp_fsm:login(Pid,"mike","secret").
%% {ok,"235 Authentication succeeded\r\n"}
%% 6> smtp_fsm:sendemail(Pid,"michael.bradford@t-mobile.uk.net",
%%   "test@test.co.uk", Msg).
%% smtp_fsm,190 : mail_from ok
%% smtp_fsm,201 : rcpt_to ok
%% smtp_fsm,212 : message ok
%% ok
%% 7> smtp_fsm:close(Pid).
%% ok
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
-module(smtp_fsm).
...
-define(CONNECT_TMO,10000). %% millisecs

%%-define(debug,on).
-ifndef(debug).
-define(DBG(F,A),true).
-else.
-define(DBG(F,A),io:format("~p,~w : "++F,[?MODULE,?LINE]++A)).
-endif.
...
-export([code_change/4,handle_info/3]).

code_change(_,_,_,_) -> to_get_rid_of_warning.

handle_info(Info,StateName,State) ->
    Reason = {StateName,Info},
    {stop,Reason,State}.
...
getFQDN()->
    ...
    {hostent,Fqdn,_,inet,_,_IpAddrs} = HostEnt,
    Fqdn.
...
start(Host,Port) ->
    gen_fsm:start(?MODULE,[Host,Port], [{timeout,infinity}]).

%% same but linked
start_link()     -> start_link("localhost", 25).
start_link(Host) -> start_link(Host, 25).
start_link(Host,Port) ->
    gen_fsm:start_link(?MODULE, [Host,Port], [{timeout,infinity}]).
...
mail_from(Fsm,Address) -> 
    gen_fsm:sync_send_event(Fsm,{mfrom,Address},infinity).
%% use rcpt_to/2 to send SMTP rcpt to:
rcpt_to(Fsm,Address) -> 
    gen_fsm:sync_send_event(Fsm,{rcpt_to,Address},infinity).
%% use message/2 to send email content
message(Fsm,Message) -> 
    case gen_fsm:sync_send_event(Fsm,data,infinity) of
...
init([Server,Port]) ->
    ?DBG("connecting to ~p\n",[Server]),
    Args = [list,{packet,0}],
    case gen_tcp:connect(Server,Port,Args,?CONNECT_TMO) of
	{ok,Socket} ->
	    ?DBG("socket open\n",[]),
...
handle_event(close, _StateName, Info) ->
