...
test_doc_creation_without_id(_Config) ->
    ?line {ok, [{<<"ok">>, true}, {<<"id">>, BinId}, {<<"rev">>, BinRev}]} = ecouch:doc_create("ecouch_ct_test", [{<<"foo">>, 1}]),

    ?line {ok, {{_, 200, _}, _, Body}} = test_helper:get("ecouch_ct_test", binary_to_list(BinId)),
    
    ?line [{<<"_id">>, BinId}, {<<"_rev">>, BinRev}, {<<"foo">>, 1}] = engejson:decode(Body),
    ok.
...
test_doc_creation_with_id(_Config) ->
    ?line {ok, [{<<"ok">>, true}, {<<"id">>, BinId}, {<<"rev">>, BinRev}]} = 
        ecouch:doc_create("ecouch_ct_test", "foo_has_one", [{<<"foo">>, 1}]),
...
test_doc_get(_Config) ->
    Body = [{<<"foo">>, 2}],
    JsonBody = engejson:encode(Body),
    ?line {ok, {{_, 201, _}, _, _}} = test_helper:put("ecouch_ct_test", "foo_has_two", JsonBody),
    ?line {ok, [{<<"_id">>, <<"foo_has_two">>}, {<<"_rev">>, _}, {<<"foo">>, 2}]} = ecouch:doc_get("ecouch_ct_test", "foo_has_two").
    
test_doc_get_all() ->
    [{userdata, {doc, "Get all of the documents from an existing database."}}].
test_doc_get_all(_Config) ->
    JsonBody1 = engejson:encode([{<<"foo">>, 1}]),
    JsonBody2 = engejson:encode([{<<"foo">>, 2}]),
    ...
    ?line {ok, [{<<"total_rows">>, 2}, {<<"offset">>, 0}, {<<"rows">>, Docs}]} = ecouch:doc_get_all("ecouch_ct_test"),
    Keys = lists:map(fun(DocProps) -> proplists:get_value(<<"key">>, DocProps) end, Docs),
